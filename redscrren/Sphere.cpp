...
Sphere::Sphere()
{
	...
	glGenVertexArrays(1, &VAOsp);
	glBindVertexArray(VAOsp);

	glGenBuffers(1, &VBOsp);
	glBindBuffer(GL_ARRAY_BUFFER, VBOsp);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);

	glGenBuffers(1, &EBOsp);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBOsp);
