...
public:
	...
	Model(std::string path, camera* camera){
		...
		this->program = ShaderLoader::CreateProgram("Model.vs", "BlingPhongShader.fs");
	...
	void Render(glm::vec3 modelpos, glm::vec3 modelscale, glm::mat4 rotationZ)
	{
		for (GLuint i = 0; i < this->meshes.size(); i++) {

		//printf("mesh size: %d \n", meshes.size());

			this->meshes[i].Render(Camera, program, modelpos, modelscale, rotationZ);
		}
	}
